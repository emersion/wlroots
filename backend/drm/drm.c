...
static bool drm_connector_commit(struct wlr_output *output) {
	...
	if (enable) {
		if (output->pending.committed & WLR_OUTPUT_STATE_MODE) {
			...
			wlr_mode = output->current_mode;
			assert(wlr_mode != NULL);
		}
	}
	...
	if (output->pending.committed &
		...
		if (enable) {
			...
			conn->crtc->pending_modeset = true;
			conn->crtc->pending.active = true;
			conn->crtc->pending.mode = mode;
		}
	}
	...
	if (output->pending.committed & WLR_OUTPUT_STATE_BUFFER) {
		...
		if (!drm_fb_import(&primary->pending_fb, drm, output->pending.buffer,
				&primary->formats)) {
			return false;
		}
	}
	...
	bool ok;
	if (output->pending.committed & WLR_OUTPUT_STATE_BUFFER) {
		ok = drm_crtc_page_flip(conn);
	} else {
		ok = drm_crtc_commit(conn, 0);
	}
	if (!ok) {
		return false;
	}
...
static struct wlr_drm_format *pick_plane_render_format(
	...
	// TODO: fallback to XRGB8888 when this function ends up being used for the
	// primary plane
	...
	if (plane_format == NULL) {
		wlr_log(WLR_DEBUG, "Plane doesn't support format 0x%"PRIX32, fmt);
		return NULL;
	}
...
static bool drm_connector_set_cursor(struct wlr_output *output,
	...
	if (buffer != NULL) {
		...
		if (drm->parent) {
			struct wlr_drm_format *format = pick_plane_render_format(drm, plane);
			if (format == NULL) {
...
static const struct wlr_drm_format_set *drm_connector_get_dmabuf_cursor_formats(
...
static const struct wlr_drm_format_set *drm_connector_get_dmabuf_primary_formats(
		struct wlr_output *output) {
	struct wlr_drm_connector *conn = get_drm_connector_from_output(output);
	if (!conn->crtc) {
		return false;
...
static const struct wlr_output_impl output_impl = {
	...
	.get_dmabuf_primary_formats = drm_connector_get_dmabuf_primary_formats,
};

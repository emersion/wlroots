...
static void finish_buffer(struct wlr_drm_dumb_buffer *buf) {
	...
	if (buf->drm_fd >= -1) {
		...
		drmIoctl(buf->drm_fd, DRM_IOCTL_MODE_DESTROY_DUMB, &destroy);
	}
...
static struct wlr_drm_dumb_buffer *create_buffer(
	...
	// Validated in `allocator_create_buffer` that the format bpp is 32
	create.bpp = 32;

	if (drmIoctl(alloc->drm_fd, DRM_IOCTL_MODE_CREATE_DUMB, &create) != 0) {
		wlr_log(WLR_ERROR, "failed to create drm dumb buffer: %s",
				strerror(errno));
		goto create_err;
	}
	...
	if (drmIoctl(alloc->drm_fd, DRM_IOCTL_MODE_MAP_DUMB, &map) != 0) {
		wlr_log(WLR_ERROR, "failed to map drm dumb buffer: %s",
				strerror(errno));
		goto create_destroy;
	}

	buffer->data = mmap(NULL, create.size, PROT_WRITE, MAP_SHARED,
			alloc->drm_fd, map.offset);
	if (buffer->data == MAP_FAILED) {
		wlr_log(WLR_ERROR, "failed to mmap drm dumb buffer: %s",
				strerror(errno));
		goto create_destroy;
	}
	...
	if (drmPrimeHandleToFD(alloc->drm_fd, buffer->handle, DRM_CLOEXEC,
			&prime_fd) != 0) {
		wlr_log(WLR_ERROR, "failed to get prime handle from GEM: %s",
				strerror(errno));
		goto create_destroy;
	}
...
static struct wlr_buffer *allocator_create_buffer(
	...
	if (drm_format->format != DRM_FORMAT_ARGB8888
